...
lpmc_stats <- ggbarplot(LPMC_df_hashtag, x = "celltypes", y = "cells", add = "mean_se",
  ...
  geom_jitter(aes(x = celltypes, y = cells, color = experiment),
              data = LPMC_df_hashtag,
              size = 2, alpha = 0.5,
...
LPMC_Osm <- FetchData(LPMC, vars = c('Osm', 'experiment', 'celltypes'))

LPMC_Osm <- LPMC_Osm %>% filter(Osm > 0)

LPMC_Osm_df <- LPMC_Osm %>% group_by(experiment, celltypes) %>% 
  summarise(Nb = n()) %>%
  mutate(C = sum(Nb)) %>%
  mutate(percent = Nb/C*100)


lpmc_plot_abs <- ggplot(LPMC_Osm_df, aes(x = experiment, y = Nb, fill = celltypes))+
...
lpmc_plot_pers <- ggplot(LPMC_Osm_df, aes(x = experiment, y = percent, fill = celltypes))+
  scale_x_discrete(limits = c('LPMCs_G3', 'LPMCs_G2')) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=c('green4', 'hotpink3', 'pink', 'cornflowerblue', 'blue', 'lightblue',
                             'turquoise1', 'deepskyblue4', 'slategray3', 'slategray1', 'cornsilk', 
